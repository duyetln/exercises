...
class DepthFirstTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @path = [@root, @node1, @node2, @node4, @node5, @node6, @node7, @node3]
  end
  
  def test_depth_first_tree_traverse
    path = []
    depth_first_tree_traverse @root do |node|
      path << node
    end
